...
describe('NoticeBar', () => {
  ...
  it ('should render correct contents', () => {
    const wrapper = shallow(<NoticeBar>test</NoticeBar>)
    expect(wrapper.find('.afo-notice-bar__content').text()).toBe('test')
  })
  // 测试不可以滚动
  it ('should not scroll', () => {
    const wrapper = mount(<NoticeBar scrollable={false}>test</NoticeBar>)
    expect(wrapper.find('.afo-notice-bar__content').html()).toBe('<div class="afo-notice-bar__content " style="padding-left: 0px;">test</div>')
  })
  // 测试props
  it ('should props', () => {
    const wrapper = mount(<NoticeBar mode="link" color="#fff" background="#ccc" fps={40} delay={10}>测试测试测试测试测试测试测试测试测试测试测试测试测试测试测试测试测试测试测</NoticeBar>)
